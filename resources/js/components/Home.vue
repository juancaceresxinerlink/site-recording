...
-->
            ...
            <div class="row mt-4 mb-4">
                ...
                <div class="col-12 col-md-2 text-right">
                    <button
                        type="button"
                        class="btn btn-xinerlink btn-block"
                        @click="search"
                    >
                        Buscar
                    </button>
...
import { numeric, alphaNum, email } from "vuelidate/lib/validators";
...
export default {
    ...
    methods: {
        ...
        getResultMessage(searchFilters, download) {
            if ("duration_max" in searchFilters)
                delete searchFilters.duration_max;
            ...
            axios
                ...
                .then((result) => {
                    ...
                    if (this.organization_id === 2) {
                        this.urlReport =
                            "https://datastudio.google.com/s/ujJN6XIDHZo";
                    } else if (this.organization_id === 3) {
                        this.urlReport =
                            "https://datastudio.google.com/s/mxAhXZMK7ac";
                    }
                })
                .catch((error) => {
                    console.log(error.response.status);
                ...
                if (this.recording[itemKey]) {
                    if (itemKey == "from" || itemKey == "to") {
                        ...
                        if (this.recording[itemKey].trim())
                            this.searchedFilters[itemKey] =
                                this.recording[itemKey].trim();
                    } else {
        ...
        search() {
            ...
            if (this.recording.duration)
                this.getDuration(this.recording.duration);
        ...
        showPlaybackModal(recording) {
            this.isLoading = true;
            this.getAudio(recording.id).then((audio_file) => {
                if (audio_file) {
